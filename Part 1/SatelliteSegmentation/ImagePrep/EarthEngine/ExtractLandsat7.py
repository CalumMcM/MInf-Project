...
def getImages(feature):
  ...
  image = ee.ImageCollection('LANDSAT/LE07/C01/T1_TOA').filterDate('2001-10-01', '2002-03-31').filterBounds(geo).sort('CLOUD_COVER').first(); # September -> April
...
def visualExtract():
    ...
    if args.outputType == "RGB":
        ...
        for i in range(args.seed, N):
            ...
            biomeVisual = getBiomeImage(geometryVisual, 'ff0aad', 'Visual', True, i, num_points)

            visCollection = ee.FeatureCollection([biomeVisual]);

            if (ee.Algorithms.IsEqual(visCollection.size(), 1)):
                export(biomeVisual, 'Border Margin', i)
    ...
    if args.outputType == "RGB":
        ...
        for i in range(args.seed, N):
            ...
            if (ee.Algorithms.IsEqual(catCollection.size(), 1)):
                export(biomeCaatinga, 'Caatinga Quad '+str(args.QuadNum), i)

            if (ee.Algorithms.IsEqual(cerCollection.size(), 1)):
                export(biomeCerrado, 'Cerrado Quad '+str(args.QuadNum), i)

            if (ee.Algorithms.IsEqual(amaCollection.size(), 1)):
                export(biomeAmazonia, 'Amazonia Quad '+str(args.QuadNum), i)
