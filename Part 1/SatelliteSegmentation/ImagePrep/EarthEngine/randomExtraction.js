...
function ExtractCerrado(){
  ...
  var random_points = 500;
  var random_points_FC = ee.FeatureCollection.randomPoints(geometryCER,random_points, 0, 10);
  
  var random_points_list = random_points_FC.toList(random_points)

  var x1s = ee.List([])
  //var x1 = random_points_FC.first().geometry().coordinates().get(0)
  ...
  var getImages = function(feature, list) {
  ...
  var geo = ee.Geometry.Polygon(geometries.first().geometry().coordinates().get(0))
  
  print (geo)
  Map.centerObject(geo, 9);
  Map.addLayer(geo)

  var images = geometries.map(getImages, true)
...
var cerradoBatch = ExtractCerrado();


var cerradoNDVIsFC = ee.FeatureCollection([ee.Feature(null, {'NDVI': cerradoNDVIs})])

print (cerradoNDVIsFC)
Map.addLayer(cerradoBatch)

Export.table.toDrive({
  collection: cerradoNDVIsFC,
